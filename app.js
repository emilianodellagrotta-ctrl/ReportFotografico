...
function handlePhotoUpload(event) {
  ...
  reader.onload = (e) => {
    ...
    img.onload = () => {
      ...
      currentTags = {}; // Initialize as empty object, not array
...
function handleFormSubmit(event) {
  ...
  const visibleTags = Object.values(currentTags).filter(tag => tag && tag.visible);
  ...
  const activeColors = Object.keys(currentTags).filter(letter => currentTags[letter] && currentTags[letter].visible);
  
  console.log('Creating report with tags:', tags);
...
function exportToJSON() {
  ...
  const exportData = {
    ...
    reports: reports.map((report, index) => {
      // CRITICAL: Export fotoOriginal (base photo without arrows)
      // This allows tags to be regenerated on import at exact positions
      const photoToExport = report.fotoOriginal || report.foto;
      
      console.log(`Export Report ${index + 1}:`);
      console.log('  - Tags:', report.tags ? report.tags.length : 0);
      console.log('  - Active colors:', report.activeColors ? report.activeColors.join(', ') : 'none');
...
function handleImportFile(event) {
  ...
  reader.onload = (e) => {
    try {
      ...
      console.log('List title:', importedData.listTitle);
      console.log('Export date:', importedData.exportDate);
      console.log('Number of reports:', importedData.reports ? importedData.reports.length : 0);
      
      // Validate data structure
      if (!importedData.listTitle || !Array.isArray(importedData.reports)) {
        console.error('Invalid JSON structure - missing listTitle or reports array');
      ...
      const isValid = importedData.reports.every((r, index) => {
        console.log(`\n--- Validating Report ${index + 1}: ${r.id} ---`);
        
        // Check for fotoDataUrl (new format) or fotoBase64 (legacy format)
        const photoData = r.fotoDataUrl || r.fotoBase64;
        const hasRequiredFields = r.id && r.dataCreazione && r.stato && photoData;
        
        if (!hasRequiredFields) {
          console.error('Missing required fields:');
          if (!r.id) console.error('  - id missing');
          if (!r.dataCreazione) console.error('  - dataCreazione missing');
          if (!r.stato) console.error('  - stato missing');
          if (!photoData) console.error('  - photo data missing');
          invalidReports++;
        } else {
          // Validate photo format
          if (!photoData.startsWith('data:image/')) {
            console.error('  - Invalid photo format (does not start with data:image/)');
            console.error('  - Photo preview:', photoData.substring(0, 50));
            invalidReports++;
            return false;
          } else if (photoData.length < 1000) {
            console.warn('  - Photo data seems very short:', photoData.length, 'chars');
          } else {
            console.log('  - Photo OK:', photoData.length, 'chars');
            validReports++;
          }
        }
      ...
      console.log(`\nValidation complete: ${validReports} valid, ${invalidReports} invalid`);
...
function confirmImport(replace) {
  ...
  try {
    console.log('\n=== PROCESSING IMPORT WITH TAG RESTORATION ===' );
    console.log('Mode:', replace ? 'REPLACE' : 'ADD');
    console.log('Reports to process:', pendingImportData.reports.length);
    
    // Process imported reports - REGENERATE photos with arrows from tag data
    const importedReports = pendingImportData.reports.map((r, index) => {
      console.log(`\n--- Processing Report ${index + 1}/${pendingImportData.reports.length}: ${r.id} ---`);
      
      // Load base photo (without arrows)
      const photoData = r.fotoDataUrl || r.fotoBase64;
      ...
      if (!photoData) {
        ...
        console.log('Photo loaded:', photoData.length, 'chars');
      }
      
      // CRITICAL FIX: Reconstruct tags object and regenerate photo with arrows
      const tagsObj = {};
      if (r.tags && Array.isArray(r.tags) && r.tags.length > 0) {
        console.log('Restoring', r.tags.length, 'tags:', r.tags.map(t => t.letter).join(', '));
        r.tags.forEach(tag => {
          tagsObj[tag.letter] = {
            x: tag.x,
            y: tag.y,
            ...
            element: null // Not needed for rendering
          };
        });
      }
      
      // Generate photo with arrows based on stored tag positions
      let photoWithArrows = photoData;
      if (photoData && Object.keys(tagsObj).length > 0) {
        console.log('Regenerating photo with arrows...');
        photoWithArrows = generatePhotoWithArrows(photoData, tagsObj);
        console.log('Photo regenerated with arrows');
      } else {
        console.log('No tags to restore, using original photo');
      }
    ...
    // Count valid photos (but don't block import)
    const validPhotos = importedReports.filter(r => r.foto && r.foto.startsWith('data:image/')).length;
    const invalidPhotos = importedReports.length - validPhotos;
    
    console.log(`Photos: ${validPhotos} valid, ${invalidPhotos} missing/invalid`);
    
    if (invalidPhotos > 0) {
      console.warn(`WARNING: ${invalidPhotos} report(s) have missing or invalid photos - continuing anyway`);
    }
    ...
    if (replace) {
      ...
      console.log('Report counter updated to:', reportCounter);
    ...
    showToast('Errore nell\'importazione: ' + error.message);
  }
